...
def current_utc():
    return datetime.now(datetime.now.utc).replace(microsecond=0).isoformat() + 'Z'
...
def load_tasks():
    ...
    try:
        ...
        if not isinstance(data, list):
            # backup and reset if structure unexpected
...
def save_tasks(tasks):
    dirpath = os.path.dirname(os.path.abspath(JSON_FILE)) or '.'
    tmp = None
    try:
        fd, tmp = tempfile.mkstemp(prefix='tasks_', suffix='.json', dir=dirpath, text=True)
...
def list_tasks(filter_status=None):
    ...
    if filter_status:
        if filter_status not in STATUS:
            print(f"Error: Invalid filter '{filter_status}'. Allowed: {', '.join(sorted(ALLOWED_STATUSES))}")
            sys.exit(1)
